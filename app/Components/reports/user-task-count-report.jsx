...
export default class UserTaskCountReport extends React.Component {
  ...
  async getReportData(e) {
    ...
    let userId =
      ...
      this.state.userNameToId[
        this.state.hiddenUserId.toLowerCase().replace(/ +/g, "")
      ];
    ...
    let { response, err } =
      await UserTaskCountReportService.getUserTaskCountReport(reportParams);
    if (err) {
      this.setState({
        message: "Error: " + err,
      });
      ...
      for (let i = 0; i < keys.length; i++) {
        ...
        response.data.data[0][keys[i]].userName =
          names && names.length > 0 ? names[0].name : "";
        ...
        <div className="container bg-white">
          ...
          <form onSubmit={this.getReportData} className="form-wrapper">
            <div className="row">
              <div className="form-group col-sm-4">
                ...
                />
                <datalist id="data">{users}</datalist>
              </div>
            ...
            <div className="row">
              ...
              <div className="form-group col-sm-3 col-lg-2">
                <input
                  ...
                  disabled={
                    this.state.month !== -1
                      ? this.state.year === -1 || this.state.year === -1
                      : this.state.year === -1
                  }
                />
              </div>
          ...
          {this.state.isLoaded ? (
            ...
            <p className="text-center mt-5">
              <strong> Please select a Member to view the report</strong>
            </p>
          )}
        </div>
      </React.Fragment>
