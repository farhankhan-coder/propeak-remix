...
export default class TaskReport extends React.Component {
  ...
  state = {
    ...
    message: "",
    noRecordsMsg: "",
    projectName: this.props.context && this.props.context.state ? this.props.context.state.projectName : '',
    data: [],
    isLoaded: true,
    headers: [
      {
  ...
  async getReportData(e) {
    ...
    let { response, err } = await TaskReportService.getMonthlyTaskReport(
      reportParams,
      projectId
    );
    if (err) {
      ...
      let data =
        ...
        response.data.data.map((t) => {
          t.startDate =
            ...
            t.startDate !== "" &&
            t.startDate !== null
              ? DateUtil.DateToString(t.startDate)
              : "";
          t.endDate =
            t.endDate !== undefined && t.endDate !== "" && t.endDate !== null
              ? DateUtil.DateToString(t.endDate)
              : "";
          return t;
        });
      this.setState({
  ...
  render() {
    const {monthlyTaskReport}=this.props
    const years = this.state.years.map((y) => {
      return (
        <option key={y * 0.5} value={y}>
          {y}
        </option>
      );
    });
      ...
      <React.Fragment>
        <div className="container bg-white">
          {this.props.projectId ? (
            <h3 className="project-title">{this.state.projectName} Report</h3>
          ) : (
            <h3 className="project-title">
              Task Report by :{" "}
              {this.state.showreport === "monthwise"
                ? this.state.monthwise
                : this.state.datewise}
            </h3>
          )}
          ...
          <div className="row">
            <div className="col-sm-12">
              <div className="mt-3">
                ...
                <label className="input-container ml-3">
                  By Date
                  <input
                    type="radio"
                    name="showreport"
          ...
          <form onSubmit={this.monthlyTaskReport} className="form-wrapper">
            <div className="row">
