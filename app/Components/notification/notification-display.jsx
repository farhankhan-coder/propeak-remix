...
export default class NotificationDisplay extends Component {
  ...
  async onHide(notificationId) {
    ...
    let { response, err } = await notificationservice.addHideNotification(
      notificationId,
      userId
    );
    if (err) {
      ...
      let notificationCopy = this.props.context.state.notifications.filter(
        (cat) => {
  ...
  render() {
    let notifications =
      ...
      this.state.notifications.filter((n) => {
        let date = dateUtil.DateToString(new Date().toISOString());
        return (
          (n.fromDate === date &&
            (n.toDate === date || !n.toDate || n.toDate > date)) ||
          (n.fromDate < date &&
            (!n.toDate || n.toDate > date || n.toDate === date))
        );
      });
    ...
    for (let i = 0; i < filterNotification.length; i++) {
      data.push(
        <div className="alert alert-info" key={i + 1}>
          {filterNotification[i]["notification"]} &nbsp;
          <span
        ...
        <React.Fragment>
          <div className="notification-container m-2">
            <div className="notification-wrapper-outer">
              {this.state.showNotifcationBox ? (
                <div className="notification-wrapper rounded">
                  <div className="title mt-2">
                    <h3 className="noti-title">
                      Broadcast{" "}
                      <span className="notification-star">
                        <span className="notification-num">{data.length}</span>
                      </span>{" "}
                      <span
                        className="float-right close"
    ...
    return <div className="row">{notificationData}</div>;
  }
}
