...
export default class NotificationForm extends React.Component {
  ...
  async getNotificationById() {
    let { response, err } = await notificationservice.getNotificationById(
  ...
  validateField(fieldName, value) {
    ...
    switch (fieldName) {
      case "notification":
        ...
        fieldValidationErrors.notification = notificationValid
          ? ""
          : " Please fill the";
        break;
      case "fromDate":
        fromDateValid = value.length !== 0;
        fieldValidationErrors.fromDate = fromDateValid
          ? ""
          : " Please fill the";
        break;
      default:
  ...
  async onNotificationSubmit(notification, projectId) {
    let { response, err } = await notificationservice.addNotification(
      notification,
      projectId
    );
    if (err) {
  ...
  async editNotification(notification) {
    let { response, err } = await notificationservice.editNotification(
      notification
    );
    if (err) {
      this.setState({
  ...
  render() {
    var { notification, toDate, fromDate } = this.state.notificationObject;
          ...
          <form className="form-wrapper" onSubmit={this.onSubmit}>
            ...
            <div className="row">
              <div className="col-sm-4">
                <div className="form-group">
                  <label htmlFor="Notification" style={labelStyle}>
                    Notification
                  </label>
            ...
            <div className="row">
              <div className="col-sm-2 offset-sm-8 ">
                <input
                  ...
                  value="Submit"
                  disabled={
                    this.state.formValid ? "" : !(notification && fromDate)
                  }
                />
              </div>
              <div className="col-sm-2 offset-sm-12">
                {this.props.projectId !== "000" ? (
                  <Link
                    to={"/notification/" + this.props.projectId}
