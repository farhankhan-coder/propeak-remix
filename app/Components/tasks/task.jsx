...
//   };
  ...
  showSubTask(id) {
    ...
    obj.className =
      obj.className === "hide-subtask" ? "show-subtask" : "hide-subtask";
  }
  ...
  onDrop(targetTaskId, targetCat, e) {
    ...
    if (this.props.onReorderTask)
      this.props.onReorderTask(srcTaskId, targetTaskId, targetCat);
  }
  ...
  async getLeaveDetails(name) {
    let userId =
      this.props.userNameToId &&
      this.props.userNameToId[name.toLowerCase().replace(/ +/g, "")];

    let { response, err } = await leaveApplicationService.getUserOnLeaveDetails(
      userId
    );
 ...
 const { task } = this.state;
    ...
    var bodyClass = task.completed
      ? "task-body task-completed"
      : "task-body " + task.taskColor;
    ...
    let displayNamePriority =
      this.props.taskPriority && this.props.taskPriority[task.priority];
    ...
    if (subTasks && subTasks.length > 0) {
      ...
      console.error("subTasks is either undefined, null, or empty.");
  }
      ...
      for (var i = 0; i < subTasks.length; i++) {
        if (subTasks[i]._id !== subTask._id) {
          tsubtasks.push({ id: subTasks[i]._id, title: subTasks[i].title });
        }
      }
      return (
        <li key={subTask._id}>
              ...
              <form>
                ...
                <div className="row">
                  <div className="col-sm-12 col-md-12">
                    <div className="form-group">
                      <label htmlFor="Dependency Subtask" style={labelStyle}>
                        Subtask Dependency
                      </label>
                      <input
                        type="text"
                ...
                <div className="row">
                  <div className="col-sm-6 col-md-6">
                    <input
                      ...
                      onClick={this.onSubmit.bind(this, subTask)}
                  ...
                  <div className="col-sm-6 col-md-6">
                    <button
                      ...
                      onClick={this.onCancelSubtask.bind(
                        this,
                        task._id,
                        subTask._id
                      )}
            ...
            <div className="subtask-title">
                ...
                {edit ? (
                  ...
                  <span>
                    <input
                      ...
                      onChange={this.props.onToggleSubTask.bind(
                        this,
                        subTask,
                        task._id
                      )}
                    />
                    &nbsp;
                    <span title={subTask.title} className="show__overflow_dots">
                      {subTask.title}
                    </span>
                  </span>
                )}
              ...
              {userRole !== "support" ? (
                <span className="subtask-actions">
                  <span
                    onClick={this.props.changeSubtaskSequence.bind(
                      this,
                      task._id,
                      subTask._id,
                      subTask.sequence,
                  ...
                  <span
                    onClick={this.props.deleteSubTask.bind(
                      this,
                      task._id,
                      subTask._id,
                      subTask.completed
                  ...
                  {subTask.completed === false ? (
                    <span
                      onClick={this.props.onTogglesubTaskEdit.bind(
                        this,
                        subTask._id,
                        task._id
                      )}
    ...
    let formattedDesc =
      task.description && task.description.length > 0
        ? task.description
            .split("\n")
            .map((t, index) => (
              <p key={task._id + "_" + index + "_" + t.length}>{t}</p>
            ))
        : "";

    let formattedTaskTitle =
      task.title && task.title.length > 70
        ? task.title.substring(0, 70) + " ..."
        : task.title;

    let editTask = validate.validateAppLevelEntitlements(
      this.state.appLevelAccess,
      "Task",
      "Edit"
    );
    let deleteTask = validate.validateAppLevelEntitlements(
      this.state.appLevelAccess,
      "Task",
      "Delete"
    );
    let cloneTask = validate.validateAppLevelEntitlements(
      this.state.appLevelAccess,
      "Task",
      "Clone"
    );
    ...
    return (
      <div
        ...
        draggable={
          userRole === "user" && task.category === "completed"
            ? "false"
            : "true"
        }
      ...
      >
        <header className={bodyClass}>
            ...
            <div className="col-sm-12">
              <div className="maintask-title">
                <div className="task-menu-box">
                  <i
                    className="fas fa-bars mt-2"
                    onClick={this.onTaskMenuToggle}
                  ></i>
                  {this.state.showContextMenu && (
                    <div className="task-icon-container">
                      {deleteTask ? (
                        userRole === "user" && uId !== getUserId ? (
                          <span
                            className="text-danger"
                            title="Delete Task"
                            onClick={this.props.deletePermissionError}
                          >
                            <i className="far fa-trash-alt text-danger"></i>
                          </span>
                        ) : (
                      ...
                      {cloneTask ? (
                        <span
                          className="text-info"
                          title="Clone Task"
                          onClick={this.cloneTask.bind(this, task._id)}
                        >
                      ...
                      {attachments ? (
                        <span>
                          <i
                            title={filteredUploadFilesCount}
                      ...
                      {!this.props.modal && editTask ? (
                        <span
                          onClick={this.editTaskWindow.bind(this, task._id)}
                        >
                          <i className="fas fa-pencil-alt text-success"></i>
                        </span>
                      ) : (
                ...
                <span
                  className={"task_priority " + task.priority}
                  title={displayNamePriority}
                ></span>

                <span
                  className="task-title"
                  onClick={this.showSubTask.bind(this, task._id)}
                >
                  {formattedTaskTitle}
                  &nbsp;
                  {task.status === "new" ? (
                    <i className="fa fa-hourglass-start" title="New"></i>
                  ) : task.status === "completed" ? (
                    <i className="fa fa-hourglass-end" title="Completed"></i>
                  ) : task.status === "onHold" ? (
                    <i className="fa fa-hourglass" title="On Hold"></i>
                  ) : (
                    <i className="fa fa-hourglass-half" title="In Progress"></i>
                  )}
                </span>
              </div>
            </div>
            ...
            <div className="task-user">
              ...
              <span>
                <Link to={"/projects/" + userId} draggable={false}>
                  <span
                    onMouseOver={this.getLeaveDetails.bind(this, uAssignedName)}
                    onMouseOut={this.onMouseOut.bind(this)}
                    title={this.state.leave ? this.state.leave : ""}
                  >
                    {uAssignedName}
                  </span>
                </Link>
          ...
          {task.endDate ? (
            <div className="lbl-smtext">
              {" "}
              <i className="text-danger fas fa-calendar-alt"></i> &nbsp;{" "}
              <span className="text-danger">{task.endDate}</span>
            </div>
          ) : (
            ""
          )}
          ...
          {task.tag ? (
            <div className="lbl-smtext">
              ...
              &nbsp;{" "}
              <span className="text-warning">&nbsp;{task.tag}&nbsp;</span>
            </div>
          ) : (
            ""
          )}
        ...
        <div id={task._id} className="hide-subtask">
          <div>
            <span
              className={task.completed ? "task-desc task-deco " : "task-desc"}
            >
              {formattedDesc}
            </span>
            {userRole !== "support" ? (
          ...
          <div className="sub-tasks" style={{ fontSize: "14px" }}>
            <div className="row">
              <div className="col-sm-12 ">
                ...
                {userRole !== "support" ? (
                  <span
                    onClick={this.props.onToggleNewSubTask.bind(
                      this,
                      task._id,
                      task.userId,
                      -1,
