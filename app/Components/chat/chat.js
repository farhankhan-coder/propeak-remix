...
export default class Chat extends Component {
  ...
  addSubject = async (subject) => {
    ...
    let { response, err } = await chatservice.addSubject(
      subject,
  ...
  ToggleSubjectEdit = (subjectId, edit) => {
    ...
    if (this.props.projectId) {
      ...
      if (edit === true)
        this.props.context.actions.updateState("projectSubjects", editSubjects);
    } else {
      this.setState({
        subjects: editSubjects,
      });
      if (edit === true)
        this.props.context.actions.updateState("subjects", editSubjects);
    }
  };
  ...
  editSubject = async (subject) => {
    ...
    let { response, err } = await chatservice.editSubject(
      subject._id,
      subject.title
    );
    if (err) {
  ...
  render() {
    ...
    var subjectList = (
      <SubjectList
        ...
        onToggleEdit={this.onToggleEditSubject}
        onEditSubject={this.onEditSubject}
        onToggleSubjectEdit={this.ToggleSubjectEdit}
              ...
              <div className="row">
                <div className="col-sm-6">
                  <div className="form-group">
                    <label htmlFor="Add Subject" style={labelStyle}>
                      Add Subject :{" "}
                    </label>
                    <input
                      type="text"
