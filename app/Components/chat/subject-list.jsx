...
const SubjectList = (props) => {
          ...
          <div className="row">
            <div className="col-sm-9">
              {subject.edit ? (
                <input
                  type="text"
                  title="Press enter to submit and esc to cancel"
                  className="form-control"
                  placeholder="press enter to add, esc to cancel"
                  value={subject.title}
                  onChange={(e) => {
                    props.onEditSubject(e, subject._id);
                  }}
                  onKeyUp={(e) => {
                    if (e.which === 13) {
                ...
                <span title="Chat">
                  {props.projectId !== "" ? (
                    ...
                    <Link
                      to={"/discussionBoard/" + subject._id}
                      className="links"
                    >
                      <label>{subject.title}</label>
                    </Link>
            ...
            <div className="col-sm-2 pull-right">
              <small>
                <span
                  onClick={() => {
                    props.onToggleEdit(subject._id);
                  }}
                >
  ...
  return (
    <div>
      <ul className="project-list">{subjectView}</ul>
    </div>
  );
};
