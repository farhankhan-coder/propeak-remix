...
import taskUrl from "../Components/tasks/task.css";
export const links = [
  { rel: "stylesheet", href: taskUrl },
  ...
  render() {
    ...
    for (var i = 0; i < elements.length; i++) {
      ...
      const menuLink = (
        <li key={i}>
          <Link to={url}>
            {submenu ? <i className="fas fa-angle-down" style={{ marginRight: "5px" }}></i> : null}
            {title}
          </Link>
        </li>
      );
      ...
      if (submenu) {
        const subMenuItems = submenu.map((subItem, index) => (
          <li key={index}>
        ...
        adminitems.push(
          <li key={i}>
            <span className="menu-with-submenu" onClick={() => this.toggleSubMenu(i)}>
              {title}
              <i className="fas fa-angle-down"></i>
            </span>
            <ul className="submenu">{subMenuItems}</ul>
          </li>
        );
      } else {
        items.push(menuLink);
    ...
    return (
      <nav id="side-navbar" className="side-navbar  _mCS_1">
