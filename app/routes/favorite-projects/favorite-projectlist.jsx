...
import project from"../../Components/project/project.css";
import * as validate from "../../common/validate-entitlements";
export const links = () => [
  { rel: "stylesheet", href: project },
];
export default class FavoriteProjectList extends Component {
  ...
  async getAllProjects() {
    ...
    let { projects, projectErr } = await projectservice.getAllFavoriteProjects(
      userId
    );
    if (projectErr) {
      this.setState({
  ...
  async onCloneProject(projectId) {
    if (window.confirm("Are you sure you want to clone this project?")) {
      let { response, err } = await projectcloneservice.addCloneProject(
        projectId
      );
      if (err) {
        this.setState({
  ...
  async onClickRemoveFavoriteProject(projectId) {
    let { response, err } = await projectservice.updateFavoriteProject(
      projectId
    );
    if (err) {
      this.setState({
  ...
  render() {
    ...
    var projectView =
      ...
      this.state.projects.map((project) => {
        ...
        var percentageProject = ((taskCompleted / project.totalTasks) * 100)
          .toString()
          .match(/^-?\d+(?:\.\d{0,2})?/);
            ...
            <div className="project-border">
              <div className="project-body" style={{ position: "relative" }}>
                ...
                <div className=" d-flex justify-content-around bottom-box">
                  ...
                  ) : proTitle !== "completed" && project.enddate !== "" ? (
                    <div
                      className="d-flex flex-column align-self-left align-items-center bottom-box-boxes"
                      title="Days left"
                    >
    ...
    return (
      <div className="project-list-scroll p-2">
        <h4 className="project-total mt-2 pb-3">Favourite Projects</h4>
